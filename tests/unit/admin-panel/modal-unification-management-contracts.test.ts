...
// U3 extracted the preview + record-edit cluster (incl. the updateTimesheetEntry write)
// out of ClientReportModal into ReportPreview. The payload guards below now read it.
const REPORTPREVIEW = read('js/ui/ReportPreview.js');
// U6/U7: the external email/generate surface is now the unified report tab (ReportTab); the
// old ClientReportModal is a thin shim. The email-action contract below reads ReportTab.
const REPORTTAB = read('js/ui/ReportTab.js');
...
describe('U0 · §14 parity — the 16 callables the unification must preserve', () => {
  const CALLABLES: Array<{ src: string; name: string; where: string }> = [
    ...
    { src: REPORTPREVIEW, name: 'updateTimesheetEntry', where: 'ReportPreview' }, // U3: moved out of ClientReportModal
...
describe('U0 · write-path + external-action contracts (must stay byte-stable)', () => {
  it('updateTimesheetEntry (report edit) carries the accumulated editHistory + autoGenerated — byte-identical after the U3 move to ReportPreview', () => {
    expect(REPORTPREVIEW).toContain("httpsCallable('updateTimesheetEntry')");
    expect(REPORTPREVIEW).toMatch(/editHistory:\s*editHistory/);
    expect(REPORTPREVIEW).toMatch(/autoGenerated:\s*currentEntry\.autoGenerated/);
  });

  it('generateAndEmail (external, client-facing) is only reached from the explicit email button — U6/U7: on ReportTab, not the retired ClientReportModal', () => {
    // Never from init/render — a click handler on the email button, then the engine.
    expect(REPORTTAB).toMatch(/emailBtn\.addEventListener\('click',\s*\(\)\s*=>\s*this\._email\(\)\)/);
    expect(REPORTTAB).toContain('window.ReportGenerator.generateAndEmail(this.getFormData())');
  });
});
