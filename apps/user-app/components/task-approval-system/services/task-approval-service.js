...
export class TaskApprovalService {
  ...
  async createApprovalRequest(taskId, taskData, requestedBy, requestedByName) {
    try {
      ...
      // ✅ OPTIMIZATION: WhatsApp notification moved to Firestore Trigger
      // The onApprovalCreated trigger will automatically send WhatsApp in the background
      // This removes the 8-second blocking call from the frontend
      // See: functions/index.js - exports.onApprovalCreated
      console.log('📱 WhatsApp notification will be sent by Firestore Trigger (non-blocking)');
