...
**Rule:** The write goes through `writeClientWithCanonicalAggregates` (AWAITED inside the txn, so its internal read+update complete BEFORE the audit `tx.set` — reads-before-writes) passing the cleaned `services[]` — `plan` (a RESTRICTED_KEY) is re-derived via `computeClientPlan`, and the hours/minutes aggregates are recomputed. Removing `ratePerHour` is NOT an input to any hours aggregate, so totalHours/hoursUsed/hoursRemaining/minutes*/isBlocked/isCritical stay identical. **AGGREGATE-DRIFT GUARD:** a client whose STORED hours aggregates already differ from the canonical recompute is SKIPPED + flagged (`AGGREGATE_DRIFT`), so the strip provably touches ONLY `plan` + the removed rate — it never silently re-derives/flips a drifted doc's block-state (devils-advocate #1). The stale 800-based `expectedRevenue` is replaced by `pricing_missing`.
**Evidence:** backend-firebase-expert GO-WITH-CONDITIONS (ratePerHour absent from `functions/shared/` aggregate code, grep-confirmed); `migrateClientInTxn` awaits the writer + the drift guard (`aggregatesDiff`); pure-core tests E1–E2 (after-strip plan = pricing_missing, hours preserved, fixed-price sibling untouched) + apply-path test (writer awaited before audit; drift → throw, no write).
...
**Evidence:** `git diff` adds only `functions/scripts/backfill-strip-legal-hourly-rate.js`, `functions/tests/backfill-strip-legal-hourly-rate.test.js`, `functions/tests/backfill-strip-legal-hourly-rate.apply.test.js`, the rubric, and a `.gitignore` comment; full functions suite **906/906** (880 + 26 new across 2 suites).
...
- **G4 customer test:** PASS — TWO suites. Pure-core `backfill-strip-legal-hourly-rate.test.js` (19) proves the data transform against the REAL `computeClientPlan` (strip 800 → `pricing_missing` not 800×hours, hours preserved, fixed-price sibling untouched, anomaly detection, idempotency). Apply-path `backfill-strip-legal-hourly-rate.apply.test.js` (7) exercises the real calling site `migrateClientInTxn` with a mock transaction: writer is AWAITED before the audit set (the missing-`await` regression guard), cleaned services + enforce mode passed, and the noop/anomaly/drift/vanished/null branches throw without writing. 26 total; full suite 906/906.
