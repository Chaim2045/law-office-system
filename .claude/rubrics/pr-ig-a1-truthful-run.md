...
|---|---|---|
| M1 | **A run that failed to scan can never report PASS.** `PASS` requires `clientsErrored === 0`. Errors with zero discrepancies must yield `PARTIAL`. | This is the headline defect. A test must exist that fails against the pre-commit code. |
...
| M6 | **The result document carries a truthful census** — `clientsTotal`, `clientsChecked`, `clientsSkippedConfig`, `clientsEmptySkipped`, `clientsErrored`, `checksRun`, `entriesRead`, `durationMs`, `discrepanciesCount`, `schemaVersion: 2`. Internally consistent: checked + errored + skips === total. | A test must assert the arithmetic, not merely the presence of keys. |
