...
### S3 — Known/accepted: the rules drift-guard covers only the 3 helper bodies, NOT the new match block (same backlog gap as Pre-H.0.0.G S3) — the deny-suite covers behavior. The emulator deny-suite is CI-verified (Java not on the local Windows box). The backfill (employee_costs population) is a SEPARATE supervised step.
...
- **G2 rollback:** PASS — `git revert` removes the job + both callables + the rules block; the data is fully DERIVED (recompute-from-source) so orphaned `client_profitability` docs are harmless (CF-only, unread once the rule/callable are gone). Code-only, ≤5 min.
...
- **G7 security:** PASS — **security-access-expert reviewed (5-lens investigation)** + **devils-advocate MANDATORY before merge** (rules change + first production isPartner()). Read gated admin||partner at the rule AND both callables; write CF-only; the §7.6 employee-leak prevented (employee read DENY test); cost stays off the world-readable clients doc; cost VALUES never client-logged.
