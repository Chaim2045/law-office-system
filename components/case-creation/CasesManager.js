 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * @description מודול ניהול תיקים - יצירה, עדכון, תצוגה, וסטטיסטיקות
 * @version 1.0.0
 * @created 2025-12-07
 * @migrated-from js/cases.js (v1.1.0)
 *
 * @features
 ...
 *
 * @important
 * ⚠️ Single Source of Truth: תמיד משתמשים ב-window.calculateRemainingHours()
 *    במקום לקרוא את hoursRemaining ישירות!
 */
...
export class CasesManager {
  ...
  init(user) {
    ...
    window.Logger?.log('📂 CasesManager initialized for user:', user.username);
  }
  ...
  async createCase(caseData) {
    try {
      window.Logger?.log('📝 Creating new case:', caseData);
      ...
      window.Logger?.log('✅ Case created successfully:', result.data.id);
      return result.data;
  ...
  async getCases(filters = {}) {
    try {
      window.Logger?.log('📋 Fetching cases with filters:', filters);
      ...
      window.Logger?.log(`✅ Fetched ${this.cases.length} cases`);
      return this.cases;
  ...
  async getCasesByClient(clientId) {
    try {
      window.Logger?.log('📋 Fetching cases for client:', clientId);
      ...
      window.Logger?.log('✅ Fetched client/case:', clientId);
      return {
        success: true,
        client: clientData,
  ...
  async getAllCases() {
    try {
      ...
      window.Logger?.log('📋 Fetching all cases');
  ...
  async updateCase(caseId, updates) {
    try {
      window.Logger?.log('📝 Updating case:', caseId, updates);
      ...
      window.Logger?.log('✅ Case updated successfully');
      return result.data;
  ...
  createCaseCard(caseItem) {
    ...
    actions.innerHTML = `
      <button onclick="window.casesManager.viewCaseDetails('${caseItem.id}')" style="
        flex: 1;
        padding: 8px 16px;
        background: #3b82f6;
        color: white;
      ...
      ${caseItem.status === 'active' ? `
        <button onclick="window.casesManager.showUpdateCaseDialog('${caseItem.id}')" style="
          padding: 8px 16px;
          background: #10b981;
          color: white;
          border: none;
        ...
        ">
          ...
          <form id="updateCaseForm">
            ...
            <div style="display: flex; gap: 12px; margin-top: 24px;">
              ...
              <button type="button" onclick="window.casesManager.closeUpdateCaseDialog()" style="
                padding: 12px 24px;
                background: #6b7280;
                color: white;
                border: none;
...
console.log('📂 CasesManager v1.0 loaded (ES6 module)');
