/**
 * 🔍 סקריפט בדיקת בעיית טבעות תאריך יעד
 *
 * הוראות שימוש:
 * 1. פתח את index.html בדפדפן
 * 2. התחבר למערכת (כחיים)
 * 3. לחץ F12 לפתיחת Console
 * 4. העתק והדבק את כל הקוד הזה
 * 5. לחץ Enter
 * 6. תראה טבלה עם כל המשימות והבעיות
 */
...
(async function debugDeadlineIssue() {
  ...
  console.log('👤 משתמש מחובר:', window.currentUser?.email || 'לא ידוע');
  ...
  try {
    ...
    // טען את כל משימות התקציב של חיים
    ...
    snapshot.forEach((doc) => {
      ...
      const taskInfo = {
        ...
        '📋 תיאור': (task.description || 'אין').substring(0, 30),
