...
| #195 | 8/4 | fix: auto-select ignores fixed services in task creation |
...
## 7ט. Migration 001 — actualHours drift (2026-04-27)

### הבעיה
17 משימות עם `actualHours` שגוי (drift היסטורי). דוגמאות:
- חזי מאנע: actualHours=201.38, actualMinutes=11289 (אמור להיות 188.15) — פער 13.23h
- סיון אזרד: actualHours=11.88, actualMinutes=414 (אמור להיות 6.9) — פער 4.98h
- שרי פדרגרין: actualHours=27.17, actualMinutes=1810 (אמור להיות 30.17) — פער 3.0h

התגלה כשמרווה דיווחה שאינה יכולה לסיים משימה (שרית רוזנס, BpBMUVqAH91FIf1xCeC3).
המשימה: actualHours=0, actualMinutes=36 → completeTask חסם.

### שורש הבעיה
עריכות שעתון לפני 2026-03-01 (לפני שה-trigger `onTimesheetEntryChanged` נוצר):
- `updateTimesheetEntry` עדכן את הרשומה ב-`timesheet_entries` ואת `task.timeEntries` array
- אך לא עדכן ישירות `actualMinutes`/`actualHours` במשימה
- ה-trigger היה אמור לעשות זאת — אבל לא היה קיים

8 רישומים עברו עריכה במשימות הבעייתיות (UktxYzM3 של חזי מאנע: 900→1, ועוד).

### Reconciliation — 2026-04-27
- סקריפט: `functions/migrations/001_fix_task_hours_minutes.js` (קיים מ-2025-10-23, רץ לראשונה)
- **17 משימות תוקנו**, 0 שגיאות, batch אחד, 1.5s
- DRY-RUN → CHECKPOINT → EXECUTE
- Backup #1 (managed Firestore backup): 2026-04-27T05:45:20Z (קיים יומי)
- Backup #2 (JSON local): `backups/drifted-tasks-2026-04-27_*.json` עם snapshot של 17 משימות
- Audit fields על כל משימה: `_migrated`, `_migrationVersion=1`, `_migratedAt`, `_oldActualHours`

### לקוחות שתוקנו
חזי מאנע -13.23h, סיון אזרד -4.98h, שרי פדרגרין +3.00h, מזל זויץ -2.98h,
תמיר אקווע (2 משימות), אבי אליהו +1.74h, מרום הנדסה (2 משימות), מורדי דבח +0.51h,
רון מאושר +0.5h, שייקסטופ +0.42h, שרית רוזנס (מרווה) +0.6h, ליאורה מילבאום +0.3h,
חיים פרץ (2 משימות)

### Code Fix — completeTask
- `functions/budget-tasks/index.js:388-405` — הוולידציה השתנתה: `actualMinutes === 0` (במקום `actualHours === 0`)
- הודעת שגיאה: fallback ל-`taskDescription || description` (במקום `title` שהיה undefined)
- Reason: `actualMinutes` הוא מקור-אמת אמין (אומת ב-100% מול `SUM(timesheet_entries)`)

### Invariant Check חדש
`functions/scheduled/index.js` — הוסף Check 4 ל-`dailyInvariantCheck`:
- בודק `Math.abs(actualHours * 60 - actualMinutes) > 1` בכל המשימות
- תופס:
  - עריכות עתידיות שלא יעדכנו את שני השדות יחד
  - הצטברות round2 בטריגר (מקסימום ~0.5min ל-100 entries)
  - flows חדשים עתידיים שעלולים לפספס שדה אחד
- Discrepancy type: `task_actualHours_actualMinutes_drift`

### לא תקרה שוב
- ה-trigger מ-2026-03-01 ו-callables מ-2026-03-29 מעדכנים את שני השדות באותה transaction
- `dailyInvariantCheck` Check 4 יתפוס drift עתידי תוך 24 שעות
- audit fields שמורים — אפשר rollback מבוקר אם יידרש

### Follow-up פתוח
- `task.timeEntries` array עדיין מכיל ערכים ישנים ב-7 משימות (לא תוקן ב-PR זה)
  - דוגמה: חזי מאנע — array.sum=12598 vs collection.sum=11289
  - השלכה: timeline UI מציג דקות ישנות (לא משפיע על `actualMinutes`/`actualHours`)
  - דורש Migration 002 נפרד עם schema mapping מדויק
