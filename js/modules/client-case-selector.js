...
return;
            ...
            serviceStages.forEach(stage => {
              if (stage.status === 'active') {
                cardsHtml += window.renderServiceCard(
                  ...
                  caseItem,
                  {
                    // ✅ FIX: העברת service.id + stage.id לזיהוי ייחודי
                    onClick: `window.clientCaseSelectorInstances['${this.containerId}'].selectService('${this.escapeHtml(stage.id)}', 'legal_procedure', '${this.escapeHtml(service.id)}')`,
    ...
    /**
     * בחירת שירות
     * @param {string} serviceId - ID של השירות או השלב
     * @param {string} type - 'hours' או 'legal_procedure'
     * @param {string} parentServiceId - (אופציונלי) ID של השירות האב (רק להליך משפטי)
     */
    selectService(serviceId, type, parentServiceId = null) {
      console.log('🎯 selectService called:', { serviceId, type, parentServiceId });
      ...
      if (type === 'hours') {
        ...
        // ✅ FIX: אם יש parentServiceId - חפש ישירות בשירות הזה
        if (parentServiceId) {
          parentService = services.find(s => s.id === parentServiceId);
          if (parentService && parentService.stages) {
            serviceData = parentService.stages.find(s => s.id === serviceId);
          }
        } else {
          // אין parentServiceId - חפש בכל השירותים (backward compatibility)
