...
- **Pre-H.1.0 — `idNumber` on client intake** — split into two at the checkpoint (2026-06-02) after devils-advocate issued STOP on the full slice (dead admin-wizard surface; the live creator is the **User App**; the `SimpleClientDialog` unvalidated direct-write bypass; "required" incoherent vs `idType:'passport'`/`SYSTEM-INTERNAL`/legacy):
  ...
  - **Pre-H.1.0b (UI — User App)** — ⏭️ **NEXT PR.** Add the ת"ז field to the LIVE wizard `apps/user-app/js/modules/case-creation/case-creation-dialog.js` (+ `case-form-validator.js` + `buildFirebaseData`); enforce **required** there; reconcile the frontend Zod (`apps/user-app/js/schemas/index.ts` `/^\d{9}$/` → check-digit + zero-pad, cross-lang drift-guard); handle `idType:'passport'` + the `validation-script.js` test harness. Full Feature Protocol.
  - **Deferred (separate PRs, tracked):** `clients` read-access tightening (G7 — all-authenticated read today); `SimpleClientDialog` bypass (validate-or-retire); the ~127-doc backfill migration; the WhatsApp `idNumber` plaintext-echo + case-number overload; the latent `pull-request.yml` PR-title shell-injection (spun off as a separate local task 2026-06-02).
