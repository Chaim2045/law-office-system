...
>
> **⏭️ NEXT = PR4 — Profitability Dashboard UI (the FIRST major visible bud).** A NEW admin-only `apps/admin-panel/profitability.html` (clones the PR2 employee-costs trio: fail-closed `role==='admin'` render-gate, design-system tokens, `ModalManager`, PII source-guard). ONE live `onSnapshot` on `client_profitability` + an in-memory JOIN to `client.plan` (via `ClientsDataManager` — no 2nd listener, no plan snapshot). **Haim checkpoint 2026-06-15 (4 locked decisions):** (1) sortable table + per-row `recompute` + a per-case detail DRAWER (`getProfitability`); (2) per-row recompute ONLY + a "חושב לאחרונה" timestamp (no bulk callable exists); (3) **defer the alert X% — color on HOURS-vs-Plan only today** (actualCost is system-wide null → no profit delta yet); (4) ship **honest-empty** (`actualCost` "טרם הוזנה" + a coverage badge, NEVER ₪0/null/NaN) — costs+backfill come later (Haim's hands). Profit column HIDDEN ("בהמשך H.6"); active-default + status filter; page gate `role==='admin'` (stricter than the rule, per D-E); the live grid is NOT per-view audited (the `getProfitability` callable is). **Frontend-only → NO devils-advocate** (confirmed: zero rules/auth/schema change, like PR2). PR5 (`רווחיות` nav tab) stays a separate tiny PR — PR4 is reachable by direct URL meanwhile. Full Feature Protocol; **STOP before merge (Haim's hands).**
