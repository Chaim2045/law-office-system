...
| F | `syncRoleClaims` utility | [#362](https://github.com/Chaim2045/law-office-system/pull/362) | ✅ merged + deployed (2026-06-08 — partner writer + lawyer removal + uniform read-merge-write; Deploy-to-Production=success). ⏳ **Phase-1 close pending the supervised `--apply`** (writes partner to Haim+Guy) | MEDIUM | C + D + E |
...
> **✅ MERGED & DEPLOYED (PR #362, 2026-06-08) — CODE COMPLETE; ⏳ PENDING THE SUPERVISED `--apply` TO CLOSE PHASE 1.** `syncRoleClaims` ships: reconciles `employees.role` → Auth `{role}` claim (admin/partner → claim; everything else → no claim), DRY-RUN default + strict `apply===true`, lock, per-employee audit-FIRST, fail-secure `messages.toRoles` probe, **read-merge-write** everywhere (shared `claim-writer.js` primitive → F + v1 `setAdminClaim` grant/revoke + `master-admin-wrappers` updateUser, which now filters to elevated so editing→lawyer/employee CLEARS the claim). `verifyClaims` extended to scan `'lawyer'`. Reviews: backend+security=GO-WITH-CONDITIONS (9 guards), devils-advocate=GO-WITH-CHANGES (folded updateUser elevated-filter + email-lowercase + lawyer-probe-is-observability; TOCTOU lock accepted), grader=PASS (8/8 MUST). 810/810 tests; CI `ci-cd-production` for `88436c5`: Deploy-to-Production=success + Health-Check=success. **🔴 PENDING HAIM (the actual Phase-1 close): the supervised `--apply` run** — DRY-RUN in PROD console → review plan + probe → confirm `messagesWithPartnerToRoles===0` → `--apply` (writes `{role:'partner'}` to Haim+Guy, clears the 7 `{role:'lawyer'}`) → `verifyClaims` confirms. The historical scope/prerequisite below is retained for reference; the §7.5 revoke read-merge-write prerequisite is now SATISFIED in F.
