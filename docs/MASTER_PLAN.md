...
| E | Claim shape consolidation | [#357](https://github.com/Chaim2045/law-office-system/pull/357) | ✅ merged + deployed (2026-06-04 — writer-contraction; Deploy-to-Production=success; final PROD `verifyClaims` G6 evidence pending Haim) | MEDIUM | B + D |
| F | `syncRoleClaims` utility | _pending_ | ⏸️ pending — **carries E's HARD revoke read-merge-write prerequisite (§7.5)** | MEDIUM | C + D + E |
...
> **🔴 HARD PREREQUISITE carried from E (devils-advocate code review of PR #357, 2026-06-04):** the v1 `setAdminClaim` **revoke** path writes `{}` (a full `setCustomUserClaims` replace). That is correct TODAY (admins hold `role` only), but the MOMENT F introduces a `partner`/composite claim, a blanket `{}` revoke would **silently wipe the user's OTHER claims**. F MUST (a) convert the revoke path to read-merge-write (or a targeted `role`-field delete) BEFORE any composite claim can coexist with admin, and (b) add a test proving a `{}`-style revoke does NOT clobber a non-admin claim (mirror the revoke→`{}` test added in E at `claim-shape-contraction-guard.test.ts`). See `functions/auth/index.js` revoke ternary + `auth/index.js:80` (composite `{role,oldUsername}` precedent).
