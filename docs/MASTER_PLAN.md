...
>
> **🟡 H.6.c — Signature-gate wiring IN PROGRESS.** After the DPA gate resolved (2026-07-01), H.6.c wires the H.5 signature check into creation. Architecture locked (Haim, 2026-07-02): **Option B — two-phase `pending_signature`** (create → verify → release), chosen over Option A (clean staging) via design-analysis + devils-advocate. Decomposed into 5 increments (c-1…c-5), **each carrying its own Intent/checkpoint**: **c-1 ✅ MERGED ([#422](https://github.com/Chaim2045/law-office-system/pull/422), `20ae775`, 2026-07-02)** — `createClientFromSalesRecord` now creates in `status:'pending_signature'` (phase 1; grader=PASS_WITH_WARNINGS, devils-advocate=GO). c-2…c-5 not yet built: **c-2** = downstream consumers (forecast skip-set + "ממתין לחתימה" badge + fix `listUnlinkedSalesRecords` to exclude sales that already hold a `pending_signature_intents` marker — a reader-side seam left open by c-1: it moved the `sales_record_links` write to c-3 but the lister still keys on it, so an already-created pending sale re-appears as "unlinked"; the `pending_signature_intents` idempotency still prevents a duplicate create, so this is a consistency gap, not data corruption) · **c-3** = live H.5 egress + release · **c-4** = match/approve screen · **c-5** = retire old UI. Live egress (c-3) stays gated on the engineering items below (cost-cap / never-persist-`reasoning` / identifier-wiring); DPA no longer blocks. **Open product decision (deferred to the c-4 checkpoint):** the match-screen outcome model for existing-client / additional-service / not-in-tofes cases — the person-match premise depends on `idNumber`, which today covers ~1/139 existing clients (a backfill dependency).
>
