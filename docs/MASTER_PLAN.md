...
**⏭️ NEXT = H.3 — Profitability layer (§8.5, LARGE).** Plan (static, at intake) + Forecast (dynamic; `forecast.actualCost = Σ(cost × hours)`, joining `timesheet_entry_costs` server-side via the Admin SDK) + the FIRST major UI bud (profitability dashboard). Dependency H.2 = DONE. The partner-claim GRANT lands here (dashboard gates on `isAdmin() || isPartner()`; D+F built the infra). **Deferred/tracked from H.1.c (still open):** BQ-table retention/TTL policy + the `tofes_export_deadletter` drain runbook (spawn-follow-up); the `SET_EMPLOYEE_COST` salary-redaction obligation remains OPEN for the FUTURE H.8 `audit_log`→BQ export (NOT H.1.c — sales_records has no salary). **Pending Haim (non-blocking, from H.1.c):** the 2 BigQuery IAM grants (runtime SA `bigquery.dataEditor`+`jobUser`; principal-scoped `dataViewer` lock) + Cloud Scheduler "Run now" smoke → then H.1.c is verified-live.
