...
>
> **⏭️ NEXT = PR3 — Forecast (the big rock, §10 2026-06-10 decision).** A CF-only `client_profitability/{caseNumber}` collection, rule `allow read: if isAdmin() \|\| isPartner()` (the FIRST production `isPartner()` consumer — dormant/fail-secure) + a dedicated aggregation job (`dailyInvariantCheck` shape: `actualCost = Σ(live entry.minutes/60 × snapshot costPerHour)` joined by **`entryId`**, `null`≠0, un-costed-coverage %) + a `getProfitability` admin\|\|partner callable. **🛑 devils-advocate + `firestore.rules.test` drift-guard + 8-scenario deny-tests MANDATORY** (§3.8.4 — firestore.rules change). paidRevenue deferred to H.6. Full Feature Protocol (investigation → checkpoint → code → grader → PR); per the loop instruction, **STOP before the devils-advocate gate**. H.2 carry-notes (join on entryId, null≠0, backfill_approx, partner-grant lands here) are in the H.3 sub-tasks below.
