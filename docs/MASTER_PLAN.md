...
**⏭️ NEXT = H.2 — Cost foundation (§8.4, MEDIUM).** Stamp `costPerHourAtEntry` on every timesheet entry at write time (read `employee_costs` via `getEmployeeCost` from G); one-time backfill of historical entries (`functions/scripts/backfill-cost-per-hour.js`, dry-run default + `--apply`). Dependency G (employee_costs schema) is DONE. This begins the profitability layer (feeds H.3 Plan/Forecast). **Deferred/tracked from H.1.c:** BQ-table retention/TTL policy + the `tofes_export_deadletter` drain runbook (spawn-follow-up); the `SET_EMPLOYEE_COST` salary-redaction obligation remains OPEN for the FUTURE H.8 `audit_log`→BQ export (NOT H.1.c — sales_records has no salary).
