...
- **H.1.b — Pattern A `validateSalesRecordExists`** — ⏭️ **NEXT — UNBLOCKED** (the live `reachable:true` hard-gate is satisfied). Security forward must-haves (from H.1.a investigation): admin-gate + field minimization + AST/runtime no-PII-in-logs guard + Zod `.strict()` + hard-scoped collection (returns ת"ז/amounts/clientName — the real PII surface).
- **H.1.c — Pattern D BigQuery export** — ⏸️ after H.1.b. New dep `@google-cloud/bigquery` — **MUST lazy-import** (index.js fans out to 67 CFs). Must-haves: redact `SET_EMPLOYEE_COST` salary before export + principal-scoped BQ IAM + treat `raw_json` as PII.
