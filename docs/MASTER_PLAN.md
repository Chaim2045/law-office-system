...
- **2026-07-28 (System-Health + JS→TS cleanup initiative — Wave-1 map landed; WhatsApp/Twilio retired; documented for full-picture continuity, Haim-approved)**: A NEW **orthogonal cleanup + JS→TypeScript-migration track** was opened (Haim: "ריפקטור לטייפסקריפט" + "ניקוי וחקירה אמיתית" של קוד-בסיס בן 1+ שנה). **⚠️ Orthogonal — does NOT change the `H.5→H.9`/H.8 MVP sequence.** The framing plan is a **sibling** to this MASTER_PLAN (not a chapter): `docs/PLAN-SYSTEM-HEALTH-TS-MIGRATION.md` — 3 waves (MAP→DECIDE→EXECUTE), a hard delete-list safelist (§7.1) protecting MASTER_PLAN/CLAUDE/rubrics/bars, and a **surfaced-item procedure (§9)** governing how items discovered mid-cleanup are handled + recorded (its 4 recording locations include this §14 entry). Approach was validated against Anthropic's published multi-agent guidance before launch (ALIGNED, 2 adjustments folded). **Wave-1 (health map) DONE** — a read-only 9-agent Workflow (7 disjoint scanners → adversarial-verify → synthesis; 97 agents, 89 claims verified, 21 refuted) produced the SSOT `docs/HEALTH-MAP-2026-07.md`: core healthy (0 dead code in the functions deploy chain), ~50 verified dead-candidates in 9 frontend clusters, a duplication map, a graded security-debt list, a docs delete-list, and a TS-migration order. The map's adversarial-verify **prevented a wrong deletion** (`clients-fluent.html` + the Fluent stack are live/intentionally-frozen — refuted). **Security sub-finding: the 2 "criticals" both dissolved on verification** — Twilio secret in public git history (real, but the **Twilio account is deleted** → worthless) and "cleartext passwords" (the code is DEAD; a read-only prod probe found **0** employee docs with a `password` field, all 15 migrated to Firebase Auth). **First surfaced item handled per §9: WhatsApp/Twilio retirement** (Haim: "מיושן לחלוטין") — **PR-1 frontend [#474](https://github.com/Chaim2045/law-office-system/pull/474) + PR-2 backend [#475](https://github.com/Chaim2045/law-office-system/pull/475) both merged** (grader PASS/PASS_WITH_WARNINGS, devils-advocate GO on both — mandatory: >100 lines + deleted deployed CFs + admin-critical); the 4 dead CFs (`whatsappWebhook`/`sendBroadcastMessage`/`sendWhatsAppApprovalNotification`/`onApprovalCreated`) were removed from prod via supervised `firebase functions:delete` (Haim's hands — the harness safety classifier BLOCKS Claude from prod deletes/deploys, a hard constraint); post-merge Deploy-to-Production + Health-Check verified green. This closes A3 (webhook signature) + A6 (PII logs) by removal. `whatsappEnabled` employee field left INERT (no migration); `.toggle-label` + `phone`/`phoneNumber` preserved. **NEXT = Wave-2 (Haim prioritizes from the map) → Wave-3 EXECUTE** in the §8 order (security A5/A4 + password dead-code → dead-code → duplication → docs → TS). NOT a §15 bar revision (an orthogonal roadmap track, no acceptance-criteria source changed).
- **2026-07-28 (Wave-3 גל-3א SECURITY sub-wave — S1–S4; each a full-protocol PR; a `docs/HEALTH-MAP` execution-log added so progress lives in the SSOT)**: Executed the security sub-wave of the cleanup track. Each item ran investigation(read-only) → checkpoint → code → grader (+ devils-advocate where high-stakes) → Haim merges → supervised prod step where needed → post-merge verify — per the §9 surfaced-item/execution procedure. **Merged:** **S1 [#477](https://github.com/Chaim2045/law-office-system/pull/477)** — `firestore.rules` tightening: deleted the dead `sessions` (orphan; presence on Realtime DB) + `function_monitor_errors` (phantom) blocks → default-deny, and locked `function_monitor_logs` → `if false` (the unwired client POC had exposed function stack-traces to any authenticated user); added a rules deny-suite + drift-guard; **devils-advocate=GO, grader=PASS_WITH_WARNINGS**; supervised `firebase deploy --only firestore:rules` (DEV=PROD). **S2 [#478](https://github.com/Chaim2045/law-office-system/pull/478)** — deleted the dead `apps/user-app/js/modules/employees-manager.js` (loaded-but-uncalled; held the 3 cleartext-password sinks) → closes the "cleartext passwords" thread; grader=PASS. **S3 [#479](https://github.com/Chaim2045/law-office-system/pull/479)** — retired the 3 unwired `function-monitor*.js` POC modules (1436 lines; the writer behind S1's lockdown); grader=PASS. **S4 (in progress)** — A4-1 hygiene: removed the hardcoded-staff-email `DEBUG_EMAILS` debug instrumentation from `WorkloadCalculator.js` (159 lines, zero behavior change) + this §14 entry + the `docs/HEALTH-MAP` execution-log. **Verified up front (dissolved 2 scanner "criticals"):** the Twilio secret was worthless (account deleted) and there were 0 cleartext-password docs in prod (read-only probe; all 15 employees on Firebase Auth) — so the security work was attack-surface reduction + dead-code removal, not incident response. **On-plan nuance (declared):** S2/S3 are dead-code (category 3ב) executed *within* 3א because they close the security threads (the password sinks; the writer behind the locked rule) — a conscious interleave, not a direction change. **גל-3א remaining (tracked in the map's execution-log):** A4-2 (scrub `devtools/**` + `add-employee-phones.js` incl. a personal @gmail — per-file dead-vs-ops judgment), A4-3 (consolidate 4 `ADMIN_EMAILS` copies → non-committed config), A4-4 (retire the deprecated `auth.js` Layer-2 email fallback — behavioral → devils-advocate). The A4 email lists are cosmetic/defense-in-depth: the real authz gate is server-side custom-claims (`firestore.rules isAdmin()`), verified — exposure, not bypass. NOT a §15 bar revision (orthogonal cleanup, no acceptance-criteria source changed).
- **2026-07-30 (📌 §11 sync — cleanup Wave-3 waves 3ג/3ד closed + 3ה opened; the detailed SSOT is the `docs/HEALTH-MAP` execution-log)**: A "is it accurate + documented" reconciliation found §14 had not been synced with the cleanup track since 2026-07-28 (S1-S4). This entry closes that gap for cross-session continuity — the **per-PR detail lives in the `docs/HEALTH-MAP-2026-07.md` execution-log** (the wave SSOT); this is the roadmap-level roll-up. All merged to `main`, each a full-protocol PR (investigation → checkpoint → grader → devils-advocate where high-stakes → Haim merges). **⚠️ Orthogonal cleanup track — does NOT change the `H.5→H.9`/H.8 MVP sequence.**
  - **🏁 גל-3ב (dead-code) — done within the 3א security sub-wave** (S2 #478 employees-manager, S3 #479 function-monitor POC) + earlier dead-candidate deletions.
  - **🏁 גל-3ג (duplication) CLOSED** — the "close the tap" wave. Built a **shared-code build-emit mechanism** (`shared-web/` canonical sources → byte-identical emit into both admin + user roots, content-hash `?v=sh-` tokens, CI drift-guard) so cross-app copies can never silently drift again. PRs: **DP-1** (presence-system, dead) · **SHARE-1/2** (mechanism + 5 byte-identical pairs) · **SHARE-3/4** (config-loader + logger parameterized via emit-time `APP_CONTEXT`) · **SHARE-6 #491** (service-card-renderer unified; the highest-stakes — H4 §7.6 fail-secure `SHOW_FINANCIALS=(APP_CONTEXT==='admin')`, devils=GO-WITH-CHANGES) · **SHARE-7 #492** (deleted the last cross-app pair — the dead admin `client-case-selector`). Investigation's adversarial-verify **prevented a wrong deletion** (the Fluent stack is live/frozen) and a wrong unify (client-case-selector admin copy was a fossil, not live-live).
  - **🏁 גל-3ד (docs) CLOSED** — 3 PRs: **DOCS-1 #493** (22 superseded/dup docs deleted; 6 were git-hash-verified byte-dupes with a surviving organized copy) · **DOCS-2 #494** (40 historical docs consolidated into `docs/archive/` via `git mv` R100 — **resolved a discovered conflict**: the map said delete `docs/analysis`+`docs/fixes`, but `archive/README`+a rubric treated them as the archive → Haim chose ARCHIVE not delete) · **DOCS-3 #495** (retired 3 dead-feature docs to archive + Haim-approved scoped staleness banners on `SYSTEM_MAP`/`SYSTEM_STATUS` for the removed WhatsApp/function-monitor lines; changelog untouched). A **chip** was spawned: `.firebase/hosting..cache` is tracked and internal docs are served on public Firebase Hosting — a scope/exposure fix, orthogonal.
  - **🟢 גל-3ה (JS→TS migration) OPENED** — Haim chose **safe tier only** (logger → service-classification → validators → claim-writer), then reassess before the large financial-core writers (services/timesheet/addTimeToTask). **TS-1 #496** migrated `functions/shared/logger.js`→TS via **mechanism A′** (source in `functions/shared/ts/`, a scoped tsconfig emits `shared/logger.js`+`.d.ts` in place → 0 consumer-path changes; named exports; `_raw:unknown` deterministic; new anchor test). Two build-integration snags the investigation missed were caught + fixed as part of the **A′ template** for TS-2..4: (a) a `.ts` at the import path tripped src-ts's `rootDir` (TS6059) → source moved to `shared/ts/`; (b) the new location lacked ESLint TS-parser coverage → added a dedicated `eslint.config.js` block. devils-advocate=GO-WITH-CHANGES folded a **CI build-drift guard** (`pull-request.yml`: rebuild `shared/tsconfig.json` + `git diff --exit-code` on `functions/shared/*.{js,d.ts}`) so a stale committed compiled output fails CI. grader=PASS, CI green.
  NOT a §15 bar revision (orthogonal cleanup, no acceptance-criteria source changed).
- **2026-08-10 (Security red-team + authz-hardening track — Haim-requested; documented for full-picture continuity, orthogonal to the MVP sequence)**: Haim requested an authorized security red-team ("אני רוצה לשלוח סוכנים שינסו לתקוף, מבחינת האבטחה"). **⚠️ Orthogonal hardening track — does NOT change the `H.5→H.9`/H.8 MVP sequence.** A 4-attacker parallel red-team (XSS/DOM · authz/IDOR · leakage · desync) found **our own recent work (#533/#534/#535) clean — zero regression** — but surfaced **3 pre-existing CRITICALs live in prod** (not from our PRs), each verified against live code (SYSTEM_MAP was stale → verified against the running code, not the map). The order Haim locked: authz first, then XSS. **Status of the 3:**
  - **🔴→✅ CRITICAL #1 (broken function-level authz) CLOSED — [PR #536](https://github.com/Chaim2045/law-office-system/pull/536), merged + deployed-green 2026-08-10 (`821fd6f`).** 9 management CFs verified auth but not role (`user.role==='admin'`), so a non-admin employee with a valid token could invoke them **directly** (bypassing the UI) to rewrite any case — including reopening a `#535`-locked service to add hours. Added `if (user.role !== 'admin') throw permission-denied` after `checkUserPermissions` in `addPackageToService`/`addHoursPackageToStage`/`moveToNextStage`/`completeService`/`changeServiceStatus`/`deleteService`/`updatePackagePurchaseDate` (services/index.js) + `changeClientStatus`/`closeCase` (clients/index.js). Verified against live code (0 user-app callers, 0 internal callers; `'manager'` is not a real role → re-anchored test mocks to `admin`). grader=PASS_WITH_WARNINGS, devils-advocate=GO. Post-merge verify (the mandatory lesson): "🚀 Deploy to Production" success + "🏥 Health Check" ✓ (overall run reads "cancelled" only because of the non-gating E2E job).
  - **🔴 CRITICAL #2 (stored XSS in admin session) STILL LIVE — PR-SEC-B not yet built.** `showOverrideModal` (`apps/admin-panel/js/ui/ClientManagementModal.js:283/291`) interpolates a raw service-name into `innerHTML`; the `data-name` escape is defeated by a dataset decode round-trip → a `<img onerror=…>` service name runs script in the admin's browser. Chains with the addServiceToClient IDOR below. Fix planned: `escapeHtml(serviceName)` at 283/291 + `service.id`/`clientId` at `ServiceOverdraftResolution.js:656/697` + delete the dead renderers.
  - **🔴 CRITICAL #3 (addServiceToClient/createClient billing-IDOR) — PR-SEC-A2 OPENED as [PR #537](https://github.com/Chaim2045/law-office-system/pull/537); STILL LIVE in prod until Haim merges + deploys.** `addServiceToClient` + `createClient` are user-app case-creation paths that were deliberately excluded from #536's role-gate. Data-verified via a read-only diagnostic (`functions/scripts/list-management-group.js` over ADC): **15 employees · 5 admins `role='admin'`** (haim, guy, or, roi, office@=office-manager) · **10 lawyers `role='lawyer'`** · **0 with `isAdmin:true`** → `role==='admin'` is the sole live admin definition and includes the office-manager. Haim's locked scope (2026-08-10): make both **admin-only** (backend) + a separate frontend follow-up to **hide the open-case button from the 10 lawyers** in the user app. This is a **G6-breaking flow change** (today lawyers open cases themselves — built intentionally; Haim chose to remove it). Branch `fix/case-mgmt-admin-only`, rubric `.claude/rubrics/pr-sec-a2-case-mgmt-admin-only.md`. **devils-advocate=GO-WITH-CHANGES** — it caught that the branch was cut *parallel* to #536 (not stacked on it), so it was **rebased ONTO `origin/main`** (graded==shipped; #536's 9 gates verified present in the integrated tree — services=8/clients=7 `role!=='admin'` gates) + #536's deliberate-exclusion lock-test (`admin-cf-role-gate.test.js`) re-anchored (A2 reverses it, Haim-approved) + interim UX declared — all applied. **grader (re-run on the integrated tree)=PASS_WITH_WARNINGS**, full suite **1637/1637**, ESLint 0. **Remaining: Haim merges #537 → post-merge Deploy-to-Production verify → then the frontend hide-button (PR-SEC-A2-frontend).**
  Documentation gaps closed in this pass (the reason for this entry): this §14 entry + a `SYSTEM_MAP.md` correction line noting the #536 authz gates. **The `docs/MASTER_PLAN.md` is the roadmap SSOT; `SYSTEM_MAP.md` is a stale (2026-02-26) status snapshot, not the plan.** Each red-team PR ran the full Feature Protocol (parallel investigation → verify-against-whole-system → grader → devils-advocate on high-stakes). NOT a §15 bar revision (orthogonal hardening track, no acceptance-criteria source changed). **⏭️ Next: Haim merges #537 (+ post-merge Deploy-to-Production verify) → PR-SEC-B (stored XSS, not yet built) → resume MASTER_PLAN H.8 (AI chat, §8.10).**
- **2026-08-17 (Hours-capacity definition — the "phantom capacity" track; orthogonal, documented for cross-session continuity)**: **⚠️ Orthogonal data-correctness track — does NOT change the `H.5→H.9`/H.8 MVP sequence.** Closes the request Haim had raised 10-15 times ("the system sums hours of stages that were never opened and presents them as available"). **Measured, read-only, over 164 live clients: 1,804h of phantom capacity across 16 cases; 5 clients displayed as solvent while genuinely over-drawn.** Root cause: `recomputeTotalHours` + `calcClientAggregates` sum `svc.totalHours` with no stage filter, while `aggregates.js` invariant I5 *declared* "ACTIVE billable capacity" and enforced nothing. Product rulings (Haim): no hours transfer between stages or services · an exposed over-draft is DISPLAYED, never auto-blocked (**binding constraint on any future PR-5: a negative `hoursRemaining` must NOT flow into `isBlocked`**) · an explicit phantom indicator rather than a swapped denominator. Shipped as 9 PRs on branch `investigate/hours-capacity`: new SSOT `functions/shared/stage-capacity.js`, a `hoursCapacity` field written fail-open by the canonical writer and mirrored onto the `createClient` direct-create route, `plan`+`hoursCapacity` added to `firestore.rules` `clientAggregateKeys()` (closing a pre-existing H.3 gap where `plan` was browser-writable), nightly drift detection for the new field, characterization tests for the previously-untested admin recomputation, an explicit phantom line on the clients table, and a progress-bar polarity fix (it filled with `remaining/total`, so shrinking real capacity made an over-drawn client look healthier). **Also found and fixed: `addHoursPackageToStage` located its target with `findIndex(s => s.type === LEGAL_PROCEDURE)` — first match, ignoring `serviceId`; 9 clients hold 2-3 legal_procedure services with colliding stage ids, so hours could land on the wrong matter silently. Same class as #544. Probe confirmed it had never fired.** **The data migration is DONE in production (163/164 clients carry the field; `internal_office` exempt per the reconciliation loop's authoritative SKIP_CLIENTS; zero hours moved — an independent re-measurement returned figures identical to the frozen baseline).** **The CODE is NOT yet merged — the display is not deployed.** Full detail: `docs/PLAN-HOURS-CAPACITY-DEFINITION-2026-08.md`. Open: PR-4 (retire the parallel frontend recomputations) · PR-5 (contract `hoursRemaining` onto the new basis, requires a matching active-only `hoursUsed`) · `moveToNextStage:1170` first-match-on-stage-status · `ClientsDataManager:499` alert blind to a second procedure. NOT a §15 bar revision.
