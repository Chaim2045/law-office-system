...
| 3ג | PR-SHARE-6 (זה) | **איחוד service-card-renderer** (ה-PR הכי high-stakes; 4 hunks). **H1** escape `window.escapeHtml \|\| fallback-5-entity` (user שדרוג מ-3→5; אין תלות-load חדשה). **H2** = "0" (החלטת חיים מ-probe; **אינרטי ב-user** — core-utils מצליל את ה-global, admin ללא שינוי). **H3** כותרת=שם-אמיתי (שינוי תצוגת-admin מוצהר; כלום לא נשען עליו). **H4 §7.6** `SHOW_FINANCIALS=(APP_CONTEXT==='admin')` fail-secure default-deny — staff לא רואה ₪ בכרטיסייה (**שומר** המצב הקיים; probe: עובד לא רואה סכום היום). tokens per-target (admin sh-0bcd498e, user sh-2799ffb8). check-8 (render admin=₪/user=בלי). **devils-advocate=GO-WITH-CHANGES** (2 תיקוני-ניסוח נכפלו: H2-אינרטי + H4-שומר-לא-משיג; 2 chips: cases.js dead-field + כפילות calculateRemainingHours). 157 טסטים | grader=? | 🟢 בעבודה |
