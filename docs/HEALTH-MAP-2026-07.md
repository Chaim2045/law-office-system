...
| 3ג | PR-SHARE-1 (זה) | **מנגנון קוד-משותף — הקדמה + הוכחה על זוג-זהה-אחד** (`idle-timeout-manager.js`). חקירת פייבל+Opus5 → 2 apps=2 publish roots נפרדים של Netlify ⟹ אין קובץ משותף ב-reference; SSOT=מקור קנוני ב-`shared-web/` + emit-מחויב-ל-git + drift-guard ב-CI. `docs/PLAN-SHARED-CODE-MECHANISM.md` נחת. תיקון-cache-bust: skip-`sh-` tokens. **devils-advocate=STOP→GO** (תפס בלוקר CRLF/LF: token חושב על CRLF≠LF-של-CI/פרוד → תוקן: emit מנרמל LF + `.gitattributes`; token=`sh-f6669b20`=hash ה-blob). grader=? · 0 שינוי בבתים; 899 טסטים; drift-guard 8/8. **נשאר (PR-SHARE-2+):** 5 זהים → config-loader+logger (מפורמטים) → service-card (H2=החלטת חיים) → client-case-selector (חקירה) | grader=? | 🟢 בעבודה |
