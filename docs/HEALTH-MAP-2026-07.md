...
| **3ד** | PR-DOCS-2 (זה) | **איחוד-ארכיון — 40 `git mv` (R100, אפס שינוי-תוכן).** מאחד את כל המסמכים ההיסטוריים למיקום אחד `docs/archive/`: `analysis/`(14)+`fixes/`(7)+`case-creation/`(4+LEGACY-BACKUP)+bucket-מדריכים(14)+session(1). **פותר את הקונפליקט הדחוי מ-DOCS-1:** חיים הכריע (2026-07-30) **ארכיון ולא מחיקה** — מיקום-ארכיון אחד, אפס אובדן-היסטוריה. `archive/README` §5 ממפה הנכנסים + באנר-עדכון מתקן במפורש את החלטת ה-"הושארו במקומם" (2026-07-23→2026-07-30). 0 הפניות קוד/CI. safelist+TIME-TRACKING-FLOW נשמרים. dead-docs → בלי devils | grader=? | 🟢 בעבודה |
