...
| 3ג | PR-SHARE-2 (זה) | **אימוץ 5 הזוגות הזהים למנגנון** (מנצחי-dedup בטוחים): `work-hours-calculator` · `business-rules-adapter` · `holidays-cache` · `work-hours-constants` (4 byte-identical, **0 שינוי בבתים**) + `system-constants` (הבדל-הערה בלבד → נרמול header, שני apps מתכנסים לבאנר נייטרלי, 0 קוד). registry+drift-guard הורחבו ל-6 מודולים; LF-token invariant אומת; 22 refs ב-12 דפים retokenized. byte-preserving, לא admin-trusted → **בלי devils** (לפי §5). grader=? · 923 טסטים | grader=? | 🟢 בעבודה |
