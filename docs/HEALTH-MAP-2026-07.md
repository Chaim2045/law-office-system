...
| 3ג | PR-SHARE-3 (זה) | **פירמוט config-loader + הצגת מנגנון הזרקת `APP_CONTEXT`** (§2.1). admin=superset של user → מקור קנוני אחד; ההתנהגויות הייחודיות ל-admin (version tracking + get()/getVersion() + logים) מגויטות ב-`APP_CONTEXT==='admin'`; emit מזריק 'admin'/'user' per-target (tokens שונים: admin sh-b6871918, user sh-d09ed9a4). **fail-secure:** ברירת-מחדל `'user'` (הזרקה שבורה→admin-surface OFF, לא דליפה) — הבסיס למתג-הסודיות של SHARE-6. **devils-advocate=GO** + 2 המשמרות שלו נכפלו פנימה (check-5 הזרקה-גנרית שמכסה אוטומטית את SHARE-6 + check-6 guard ברירת-מחדל). admin שקול-התנהגותית; user 0 שינוי נצפה (מתודות dormant). 127 טסטים. grader=? | grader=? | 🟢 בעבודה |
