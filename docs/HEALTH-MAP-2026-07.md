...
| 3ג | #486 (DP-1) | **הכפילות ה"מסוכנת" התפוגגה באימות:** `presence-system.js` קיים ב-2 apps אבל העותק של admin **טעון-ומעולם-לא-נקרא** (0 קוראים; grep מלא ב-admin-panel); כל 4 השדות (`lastLogin`/`loginCount`/`isOnline`/`lastSeen`) נכתבים ע"י user-app (`authentication.js` + `connect()`), נקראים ע"י admin. לכן העותק של admin = **קוד מת**, לא איחוד. מוחק `apps/admin-panel/js/modules/presence-system.js` + 2 תגי `<script>`. **שומר** את העותק החי ב-user-app. 0 שינוי-התנהגות | grader=PASS | ✅ מוזג |
| 3ג | #487 (PR-SHARE-1) | **מנגנון קוד-משותף — הקדמה + הוכחה על זוג-זהה-אחד** (`idle-timeout-manager.js`). 2 apps=2 publish roots נפרדים ⟹ אין קובץ משותף ב-reference; SSOT=מקור קנוני ב-`shared-web/` + emit-מחויב-ל-git + drift-guard ב-CI. תיקון cache-bust skip-`sh-`. **devils-advocate=STOP→GO** (בלוקר CRLF/LF תוקן; token=`sh-f6669b20`=hash ה-blob). grader=PASS 7/7 · 0 שינוי בבתים; 899 טסטים | grader=PASS | ✅ מוזג |
| 3ג | #488 (PR-SHARE-2) | **אימוץ 5 הזוגות הזהים למנגנון**: work-hours-calculator · business-rules-adapter · holidays-cache · work-hours-constants (4 byte-identical, **0 שינוי בבתים**) + system-constants (נרמול header בלבד). drift-guard→6 מודולים; 923 טסטים | grader=PASS | ✅ מוזג |
| 3ג | #489 (PR-SHARE-3) | **פירמוט config-loader + הצגת מנגנון `APP_CONTEXT`** (§2.1). admin=superset; ההתנהגויות הייחודיות ל-admin מגויטות `APP_CONTEXT==='admin'`; emit מזריק per-target (tokens שונים). **fail-secure ברירת-מחדל `'user'`** = בסיס SHARE-6. devils-advocate=GO + 2 משמרות נכפלו (check-5 הזרקה-גנרית + check-6 guard ברירת-מחדל). 127 טסטים | grader=PASS | ✅ מוזג |
| 3ג | #490 (PR-SHARE-4) | **פירמוט logger** (user=superset): בלוק PROD-console-silence מגויט `APP_CONTEXT==='user' && isProduction`. **crux: admin לעולם לא משתיק console בפרוד**; check-7. **devils-advocate דילוג-מוצהר** (§5). 141 טסטים | grader=PASS | ✅ מוזג |
| 3ג | #491 (PR-SHARE-6) | **איחוד service-card-renderer** (4 hunks): H1 escape 5-entity (user שדרוג) · H2="0" (החלטת חיים; אינרטי ב-user) · H3 כותרת=שם-אמיתי · **H4 §7.6** `SHOW_FINANCIALS=(APP_CONTEXT==='admin')` fail-secure (staff בלי ₪). **devils-advocate=GO-WITH-CHANGES** (2 תיקוני-ניסוח נכפלו; 2 chips). grader=PASS 8/8; check-8 crux. **תיקון-CI:** טסט קדם-קיים escapeHtml-routing עודכן לצורת H1 (המיס: הרצתי vitest ממוקד, CI תפס) | grader=PASS | ✅ מוזג |
| 3ג | SHARE-7 (זה) | **מחיקת העותק המת של client-case-selector ב-admin — סוגר גל-3ג.** חקירת Opus: הכפילות ה-cross-app האחרונה = **לא live-live**; העותק של admin טעון-ומעולם-לא-נוצר (0 `new`, 0 container, 0 consumer). מקור: 10/2025 רפקטור "לקוח→תיקים"; היה חי 11-12/2025; התייתם כשמחקנו את דיאלוגי-admin ב-H.6.c #454. מוחק `apps/admin-panel/js/modules/client-case-selector.js` + תג `<script>` (clients.html:619). **שומר** את העותק החי ב-user (3 יצירות). 189-diff = פיצ'רים חיים ב-user (מניעת-שירות-שגוי/fixed-service) — כל admin עתידי חייב להיבנות מ-user. 0 refs שאריות. dead-code → **בלי devils** (כמו DP-1). grader=PASS 6/6; CI ירוק | grader=PASS | ✅ מוזג #492 |
| **3ד** | PR-DOCS-1 (זה) | **פתיחת גל-3ד (מסמכים) — מחיקות טהורות.** מוחק 22 מסמכים חד-משמעיים: **6 כפילי-בייט** (git-hash זהה לעותק מאורגן בתת-תיקייה שנשאר — `docs/architecture/`/`processes/`/`deployment/`) + **2 גיבויים** (`BACKUP_PROGRESS_BARS_OLD`, `.claude/README-old`) + **14 דוחות top-level** שהוחלפו ע"י המפה. אפס קוד/התנהגות/אובדן-תוכן. **קונפליקט שהתגלה+נדחה ל-PR-DOCS-2:** `docs/analysis`(14)+`docs/fixes`(7)+`CASE-CREATION`(4)+`LEGACY-BACKUP` = **הארכיון הקיים** (מופה ב-`archive/README`+`pr-docs-archive.md`) — ארכיון-מול-מחיקה = החלטת חיים ב-PR-2. **chip:** `.firebase/hosting..cache` tracked + מסמכים פנימיים מוגשים ב-Hosting ציבורי. safelist מגן. dead-docs → בלי devils. grader=PASS_WITH_WARNINGS; CI ירוק | grader=PASS_WITH_WARNINGS | ✅ מוזג #493 |
| **3ד** | PR-DOCS-2 (זה) | **איחוד-ארכיון — 40 `git mv` (R100, אפס שינוי-תוכן).** מאחד את כל המסמכים ההיסטוריים למיקום אחד `docs/archive/`: `analysis/`(14)+`fixes/`(7)+`case-creation/`(4+LEGACY-BACKUP)+bucket-מדריכים(14)+session(1). **פותר את הקונפליקט הדחוי מ-DOCS-1:** חיים הכריע (2026-07-30) **ארכיון ולא מחיקה** — מיקום-ארכיון אחד, אפס אובדן-היסטוריה. `archive/README` §5 ממפה הנכנסים + באנר-עדכון מתקן במפורש את החלטת ה-"הושארו במקומם" (2026-07-23→2026-07-30). 0 הפניות קוד/CI. safelist+TIME-TRACKING-FLOW נשמרים. dead-docs → בלי devils. grader=PASS 6/6; CI ירוק | grader=PASS | ✅ מוזג #494 |
| **3ד** | #495 (PR-DOCS-3) | **סוגר גל-3ד — באנרי doc-drift.** חקירה סיווגה 5 מסמכים שמזכירים פיצ'רים מפורקים: **ARCHIVE 3** (`FUNCTION_MONITOR_README`+`ANALYTICS_DASHBOARD_GUIDE`=POST-מת S1/S3; `WHATSAPP-PDF-UPLOAD-FEATURE`=WhatsApp מת #474/#475) — `git mv` לארכיון (0 קוד קיים, אין מה לתקן). **BANNER 2** (`SYSTEM_MAP`+`SYSTEM_STATUS`=מסמכי-חיים, רק שורות WhatsApp ישנות) — באנר-התיישנות ממוקד + סימוני `[הוסר]` inline (SYSTEM_MAP: פסקת-תיקון אחרי H.4; SYSTEM_STATUS: **אושר ע"י חיים**, 3 שורות 163/712/716, changelog היסטורי לא נגע). `archive/README` §6. safelist מגן. dead-docs → בלי devils. grader=PASS 7/7; CI ירוק | grader=PASS | ✅ מוזג #495 |
| **3ה** | TS-1 (זה) | **פותח גל-3ה (הגירת TypeScript) — `logger.js`→TS.** חיים אישר **שכבה-טהורה בלבד** (logger→service-classification→validators→claim-writer) ואז נעריך; מנגנון **A′ in-place**. **crux שהחקירה פספסה+תיקנתי:** `.ts` בנתיב-הייבוא גורם ל-src-ts לתפוס אותו על ה-`.d.ts`→TS6059(rootDir); פתרון=מקור ב-`shared/ts/logger.ts`, tsconfig ממוקד פולט `shared/logger.js`+`.d.ts` בנתיב-זהה→src-ts פותר ל-`.d.ts` (אומת נעלם). named-exports נשמרים (5 exports+`__esModule`), `_raw:unknown` מפורש=פלט דטרמיניסטי, טסט-עוגן חדש (surface+forwarding), LF ננעל. 1:1 port. **מגביל מקומי:** functions/node_modules חלקי→typecheck+ts-jest ב-CI. **devils=GO-WITH-CHANGES** (4/5 לא-נחתו; התקפה-2 נחתה+קופלה: אין drift-guard לפלט-מהודר→**הוספתי צעד CI** build+`git diff --exit-code` שמגן על כל השכבה). +תיקון CI: בלוק eslint ל-`shared/ts/**` (המיקום החדש לא היה מכוסה ב-parser). grader=PASS 8/8; CI ירוק | grader=PASS | ✅ מוזג #496 |
| **3ה** | TS-2 (זה) | **`claim-writer.js`→TS** — היעד הבטוח באמת (קובץ יחיד, `.d.ts` קיים, **אפס-מראה**, primitive אבטחה טהור: 2 פונקציות read-merge-write ל-Auth claims). **סידור-מחדש מבוסס-ממצא:** המפה שמה service-classification #2, אבל בדיקה גילתה שהוא הכי מסובך (2 CJS+2 IIFE+sync-test 5-כיווני) → claim-writer הוא ה-logger-analog האמיתי. **תבנית A′ שלמה מ-TS-1 שוחזרה ללא שינוי** (+1 שורת include, בלי eslint/CI/package חדשים). named-exports, `ClaimMap` דטרמיניסטי, 1:1 port. **טסט קיים** (`functions/tests/claim-writer.test.js`) מפַנֵּן ההתנהגות. אימות מלא מקומי (טהור→0 TS2307). **שדרוג-איכות (חיים: "הדגש על איכות"): שברתי את הדילוג-המוצהר והרצתי devils-advocate + security-access-expert** — primitive אבטחה. **security=PASS** (byte-for-behavior-identical, אפס PII/logging חדש, `.d.ts` חושף בדיוק 3 סמלים, 3 הצרכנים פותרים ללא שינוי); **devils=GO-WITH-CHANGES** (טענת "אפס שינוי-לוגיקה" עמדה בהתקפה — fresh-compile byte-identical, כל נתיב-אבטחה אומת ב-runtime). **2 חיזוקים קופלו:** (1) `newLine:"lf"` ב-`shared/tsconfig.json`→drift-guard מוגן-קומפיילר (לא תלוי-gitattributes; מיטיב גם ל-logger; אומת no-op על הפלט המקומם); (2) 3 assertions ל-role-לא-מחרוזת בטסט הקיים (`123`/`null`/`{}`)→סוגר את הפער היחיד ב-pin ההתנהגותי. **standing-rule: כל מודול אבטחה/auth בשכבה יקבל devils+security גם אם "טהור".** service-classification+validators (עם מראה) אחרי. | grader=PASS | ✅ מוזג #498 |
| **3ה** | TS-3 (זה) | **`validators.js`→TS** — 5 כלי-אימות-קלט (`sanitizeString`/`isValidIsraeliPhone`/`isValidEmail`/`isValidIsraeliId`/`getDescriptionLimit`); 6 צרכנים + ~25 mocks + 2 טסטים ישירים. **שונה מ-TS-1/2:** (א) **אין `.d.ts` קודם** → A′ מייצר את ההצהרה הראשונה; (ב) **פונקציה א-טהורה אחת** — `getDescriptionLimit` עם `require('./constants')`+`require('firebase-admin')`+קריאת Firestore. **הושארו `require()` inline** (מוקלד `NodeRequire`→`any` דרך @types/node) ולא הומרו ל-ES import — לכן הקובץ קומפל **מלא מקומית** (0 שגיאות לא-logger) למרות firebase-admin. תבנית A′ שוחזרה (+1 include). פרמטרים-שמורים מוקלדים `unknown` (guards נשארים type-necessary); `getDescriptionLimit` בחתימה מפורשת→`.d.ts` דטרמיניסטי. **completeness-checker רץ (0 קריטיים)** — אישר את ההבחנה inline-require-לא-import, ש-ה-PII-guard קורא את הפלט ב-regex-שלילי, שה-drift-guard של ת"ז הוא frontend-בלבד (וקטורים לא מקור), ו-26 ה-mocks מחליפים סיטונאית; 5 ה-guardrails יושמו. **אימות התנהגות: 96/96 מקרים זהים** (old-JS מול פלט-חדש, כולל throw-על-truthy-לא-מחרוזת + ת"ז check-digit/pad/trim). **devils דילוג-מוצהר** (לא primitive אבטחה — כלי-אימות-פורמט; אפס-לוגיקה-חדשה; מאומת מקומית מלא; 2 טסטים קיימים + drift-guard). ESLint 0 (`catch {` optional-binding). service-classification (הכי מסובך) אחרי. | grader=PASS | 🟢 PR פתוח |
