...
| **3ד** | PR-DOCS-3 (זה) | **סוגר גל-3ד — באנרי doc-drift.** חקירה סיווגה 5 מסמכים שמזכירים פיצ'רים מפורקים: **ARCHIVE 3** (`FUNCTION_MONITOR_README`+`ANALYTICS_DASHBOARD_GUIDE`=POST-מת S1/S3; `WHATSAPP-PDF-UPLOAD-FEATURE`=WhatsApp מת #474/#475) — `git mv` לארכיון (0 קוד קיים, אין מה לתקן). **BANNER 2** (`SYSTEM_MAP`+`SYSTEM_STATUS`=מסמכי-חיים, רק שורות WhatsApp ישנות) — באנר-התיישנות ממוקד + סימוני `[הוסר]` inline (SYSTEM_MAP: פסקת-תיקון אחרי H.4; SYSTEM_STATUS: **אושר ע"י חיים**, 3 שורות 163/712/716, changelog היסטורי לא נגע). `archive/README` §6. safelist מגן. dead-docs → בלי devils | grader=? | 🟢 בעבודה |
