...
exports.TIMESHEET_ENTRY_COSTS_COLLECTION = void 0;
exports.resolveEmployeeCost = resolveEmployeeCost;
exports.buildEntryCostDoc = buildEntryCostDoc;
/**
 ...
 */
async function resolveEmployeeCost(email) {
    const key = (typeof email === 'string' ? email : '').toLowerCase().trim();
    if (!key) {
        return { costPerHour: null, costSource: 'no_cost_doc' };
    }
    try {
        const snap = await admin.firestore().collection(employee_cost_1.EMPLOYEE_COSTS_COLLECTION).doc(key).get();
        if (!snap.exists) {
            return { costPerHour: null, costSource: 'no_cost_doc' };
        }
...
exports.TIMESHEET_ENTRY_COSTS_COLLECTION = 'timesheet_entry_costs';
/**
 ...
 */
function buildEntryCostDoc(entryId, employee, resolved) {
    return {
        entryId,
...
//# sourceMappingURL=resolve-employee-cost.js.map