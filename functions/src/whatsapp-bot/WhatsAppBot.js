...
class WhatsAppBot {
    ...
    async processMessage(message, session, userInfo) {
        ...
        // 1️⃣ סטטיסטיקות
        if (msgNormalized.match(/^1$|סטטיסטיק|נתונים|דוח/)) {
            return await this.showStats(userInfo, session);
        }

        // 2️⃣ שעתונים
        if (msgNormalized.match(/^2$|שעתונים|שעות|timesheets/)) {
            return await this.showTimesheetsMenu(userInfo, session);
        }

        // 3️⃣ משימות שלי
        if (msgNormalized.match(/^3$|משימות שלי|המשימות שלי|my tasks/)) {
...
1️⃣ סטטיסטיקות יומי
2️⃣ שעתונים (רישומי שעות)
3️⃣ משימות שלי (סטטוס משימות)
4️⃣ עזרה
━━━━━━━━━━━━━━━━━━━━
...
1️⃣ "סטטיסטיקות" / "1" - נתונים יומיים
2️⃣ "שעתונים" / "2" - רישומי שעות
3️⃣ "משימות שלי" / "3" - סטטוס משימות
4️⃣ "עזרה" / "4" - מדריך זה
