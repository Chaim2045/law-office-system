...
/** Per-service plan snapshot (derived from the service's own intake fields). */
export interface ServicePlan {
  /** Budgeted billable hours; `null` for fixed-price services (no hours budget). */
  expectedHours: number | null;
  /** Known revenue; `null` when the service has no derivable price (never a guess). */
  expectedRevenue: number | null;
  revenueSource: 'fixed_price' | 'rate_x_hours' | 'unknown';
}

/** Client-level Plan (the doc-stored shape; cost/profit intentionally absent). */
export interface ClientPlan {
  /** Σ budgeted billable hours over active services (fixed services contribute 0). */
  expectedHours: number;
  /** Σ of KNOWN per-service revenue ONLY — never includes a guessed/defaulted rate. */
  expectedRevenue: number;
  /** false when any active service has unknown revenue (so the total is partial). */
  pricingComplete: boolean;
  /** # active services whose revenue could not be derived (the `pricing_missing` count). */
  pricingMissingCount: number;
  /** # active (non-skipped) services the plan was computed over. */
  serviceCount: number;
 ...
 */
const PLAN_SKIP_STATUSES: readonly string[] = ['archived'];
...
type ServiceLike = Record<string, unknown> | null | undefined;
 ...
 */
function isHourlyService(svc: ServiceLike): boolean {
  if (!svc || typeof svc !== 'object') return false;
  if (svc.type === 'fixed') return false;
  if (svc.pricingType === 'fixed') return false;
  return true;
}

/** Derives one service's Plan snapshot (raw intake values; no re-derivation). */
export function computeServicePlan(svc: ServiceLike): ServicePlan {
 ...
 */
export function computeClientPlan(services: unknown): ClientPlan {
  const list: ServiceLike[] = Array.isArray(services) ? services.filter(Boolean) : [];
  let expectedHours = 0;
  ...
  for (const svc of list) {
    ...
    if (PLAN_SKIP_STATUSES.includes(status)) continue;
    serviceCount += 1;
    const plan = computeServicePlan(svc);
    if (plan.expectedHours !== null) expectedHours += plan.expectedHours;
    if (plan.expectedRevenue !== null) expectedRevenue += plan.expectedRevenue;
    else pricingMissingCount += 1;
  }
...
export const PROFITABILITY_PLAN_SCHEMA_VERSION = SCHEMA_VERSION;
/** Exposed for the drift-guard test (pins it to aggregates.NON_AGGREGATING_STATUSES). */
export const _PLAN_SKIP_STATUSES = PLAN_SKIP_STATUSES;
