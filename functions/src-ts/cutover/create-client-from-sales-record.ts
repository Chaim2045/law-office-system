...
import { REGION, TOFES_MECHER_SA_KEY_SECRET } from '../config';
...
import { computeClientPlan } from '../profitability/client-plan';
 ...
 */
export const createClientFromSalesRecordInputSchema = z
  .object({
    salesRecordId: z
      .string()
 ...
 */
async function allocateCaseNumberInTxn(
  transaction: admin.firestore.Transaction,
  db: admin.firestore.Firestore
): Promise<{ caseNumber: string; counterRef: admin.firestore.DocumentReference; counterUpdate: Record<string, unknown> }> {
    ...
    if (data.year === currentYear) {
      ...
      if (nextNumber > 999) {
        ...
        throw new HttpsError(
          'resource-exhausted',
          'הגעת למספר התיקים המרבי לשנה זו. אנא פנה לתמיכה.'
        );
      }
    }
  }
  ...
  const prevStats = (counterDoc.data()?._stats ?? {}) as { totalTransactions?: unknown };
...
function buildFixedClientData(args: {
  ...
  const {
    caseNumber, actorName, clientName, idNumber, caseTitle,
    fixedPrice, salesRecordId, serviceId, nowIso
  } = args;
  ...
  clientData.plan = computeClientPlan(services);
 ...
 */
export async function createClientFromSalesRecordHandler(
  request: CallableRequest<unknown>
): Promise<CreateClientFromSalesRecordResponse> {
  // ─── (1) Auth gate (role-only admin) ──────────────────────────────────────
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'נדרשת התחברות למערכת.');
  }
  const claims = (request.auth.token ?? {}) as { role?: string; name?: string };
  if (claims.role !== 'admin') {
    throw new HttpsError(
      'permission-denied',
      'רק מנהל מערכת רשאי ליצור לקוח ממכר.'
    );
  }
  const adminUid = request.auth.uid;

  // ─── (2) Input validation (Zod, strict) ───────────────────────────────────
  const parsed = createClientFromSalesRecordInputSchema.safeParse(request.data);
  if (!parsed.success) {
  ...
  try {
    ...
    throw new HttpsError(
      'unavailable',
      'לא ניתן לקרוא את רשומת המכר כעת. אנא נסה שוב מאוחר יותר.'
    );
  }
  ...
  if (!sale.exists) {
    ...
    throw new HttpsError(
      'failed-precondition',
      'רשומת המכר לא נמצאה במערכת המקור. ייתכן שנמחקה.'
    );
  }
  ...
  if (typeof fixedPrice !== 'number' || !Number.isFinite(fixedPrice)) {
    ...
    throw new HttpsError(
      'failed-precondition',
      'לרשומת המכר אין סכום (לפני מע"מ); לא ניתן ליצור שירות. יש להזין ידנית.'
    );
  }
  ...
  try {
    ...
    throw new HttpsError(
      'internal',
      'יצירת הלקוח מרשומת המכר נכשלה כעת. אנא נסה שוב מאוחר יותר או פנה לתמיכה.'
    );
  }
...
export const createClientFromSalesRecord = onCall<
  unknown,
  Promise<CreateClientFromSalesRecordResponse>
>({ region: REGION, secrets: [TOFES_KEY] }, createClientFromSalesRecordHandler);
