...
exports.updateTimesheetEntry = functions.https.onCall(async (data, context) => {
  try {
    ...
    console.log(`  הפרש דקות: ${data.minutesDiff}`);
    ...
    if (data.autoGenerated && data.taskId) {
      ...
      if (taskDoc.exists) {
        await taskRef.update({
          actualMinutes: admin.firestore.FieldValue.increment(data.minutesDiff),
          lastActivity: admin.firestore.FieldValue.serverTimestamp()
        });

        console.log(`  ✅ משימה ${data.taskId} עודכנה (הפרש: ${data.minutesDiff} דקות)`);
      }
